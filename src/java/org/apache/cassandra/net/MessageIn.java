...
public class MessageIn<T>
{
    ...
    private MessageIn(InetAddress from, T payload, Map<String, byte[]> parameters, MessagingService.Verb verb, int version, long creationTime)
    {
        this.from = from;
    ...
    public static <T> MessageIn<T> create(InetAddress from, T payload, Map<String, byte[]> parameters, MessagingService.Verb verb, int version, long creationTime)
    {
        return new MessageIn<T>(from, payload, parameters, verb, version, creationTime);
    }
    ...
    {
        return new MessageIn<T>(from, payload, parameters, verb, version, System.currentTimeMillis());
    }
    ...
    public static <T2> MessageIn<T2> read(DataInput in, int version, String id) throws IOException
    {
        ...
        if (payloadSize == 0 || serializer == null)
            return create(from, null, parameters, verb, version, creationTime);
        T2 payload = serializer.deserialize(in, version);
        return MessageIn.create(from, payload, parameters, verb, version, creationTime);
    }
